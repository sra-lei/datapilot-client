 ...
 * "UTF-8→CP1252/Latin-1 错编码"乱码，也能在渲染时无损还原为中文，不再要求用户删库重建。
 *
 * 修复原理：中文 UTF-8 字节流若被按 Windows-1252（CP1252，网页最常见）而非纯 Latin-1
 * 解码，0x80-0x9F 区段会变成可见字符（€‰™œ 等）。把"字符 → 字节"（Latin-1 1-1 +
 * CP1252 高半区映射表）再按 UTF-8 解码即可无损还原；修复前后做特征判定与迭代，避免破坏正常文本。
 */
